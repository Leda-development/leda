...
export const createCalendarClickHandler = ({
  ...
  Guards(type)
    .when(equals(CALENDAR_CLICK_ACTION.DATES_PREV), () => handleDatesPrevClick({
      viewDate, dateShorthand, dispatch, min, isPrevButtonDisabled,
    }))
    .when(equals(CALENDAR_CLICK_ACTION.DATES_NEXT), () => handleDatesNextClick({
      viewDate, dateShorthand, dispatch, max, isNextButtonDisabled,
    }))
    .when(equals(CALENDAR_CLICK_ACTION.DATES_SELECT), () => handleDatesSelect({
    ...
    .when(equals(CALENDAR_CLICK_ACTION.MONTHS_PREV), () => handleMonthsPrevClick({
      viewDate, dateShorthand, isDateOutOfMinYearRange, dispatch, min,
    }))
    .when(equals(CALENDAR_CLICK_ACTION.MONTHS_NEXT), () => handleMonthsNextClick({
      viewDate, dateShorthand, isDateOutOfMaxYearRange, dispatch, max,
    }))
    .when(equals(CALENDAR_CLICK_ACTION.MONTHS_SELECT), () => handleMonthsSelect({
      viewDate, dateShorthand, isDateOutOfMinDecadeRange, dispatch, min, max, monthCell: payload && payload.monthCell,
    }))
    .when(equals(CALENDAR_CLICK_ACTION.YEARS_PREV), () => handleYearsPrevClick({
      viewDate, dateShorthand, isDateOutOfMinDecadeRange, dispatch, min,
    }))
    .when(equals(CALENDAR_CLICK_ACTION.YEARS_NEXT), () => handleYearsNextClick({
      viewDate, dateShorthand, isDateOutOfMaxDecadeRange, dispatch, max,
    }))
    .when(equals(CALENDAR_CLICK_ACTION.YEARS_SELECT), () => handleYearsSelect({
      viewDate, dateShorthand, dispatch, max, min, yearCell: payload && payload.yearCell,
    }))
    .when(equals(CALENDAR_CLICK_ACTION.TITLE_CLICK), () => handleTitleClick({
      viewType, isOneMonthInRange, isOneYearInRange, dispatch,
    }))
    .when(equals(CALENDAR_CLICK_ACTION.TODAY_BUTTON_CLICK), () => handleTodayButtonClick({
