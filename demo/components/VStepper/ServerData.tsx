  ...
  <L.Div _box _inner _demoBg>
    <L.VStepper>
      {data.map((dataItem) => {
        ...
        // в statusRender передаем текст статуса: "Заполнен" для success, "Не заполнен" для progress или ещё не пройденного шага, "Ошибка" для danger
        return (
          <L.VStepper.Item
            key={dataItem.title}
            hasSignIcon
            ...
            statusRender={({
              Element,
            }) => <Element>{statusText[dataItem.status as keyof typeof statusText]}</Element>}
          >
            {dataItem.content}
          </L.VStepper.Item>
        );
